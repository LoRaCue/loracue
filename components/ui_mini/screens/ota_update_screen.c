...
void ui_screen_ota_update(void)
{
    ...
    // Title
    u8g2_SetFont(&g_u8g2, u8g2_font_helvB10_tr);
    u8g2_DrawStr(&g_u8g2, 20, 15, "Updating...");
    
    // Progress bar background
    u8g2_DrawFrame(&g_u8g2, 10, 25, 108, 12);
    
    // Progress bar fill
    uint8_t fill_width = (progress * 104) / 100;
    if (fill_width > 0) {
        u8g2_DrawBox(&g_u8g2, 12, 27, fill_width, 8);
    }
    
    // Progress percentage
    snprintf(progress_str, sizeof(progress_str), "%d%%", progress);
    u8g2_SetFont(&g_u8g2, u8g2_font_helvB08_tr);
    uint8_t str_width = u8g2_GetStrWidth(&g_u8g2, progress_str);
    u8g2_DrawStr(&g_u8g2, (128 - str_width) / 2, 50, progress_str);
    
    // Warning message
    u8g2_SetFont(&g_u8g2, u8g2_font_6x10_tr);
    u8g2_DrawStr(&g_u8g2, 10, 62, "Do not power off!");
