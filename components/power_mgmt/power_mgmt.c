...
esp_err_t power_mgmt_init(const power_config_t *config)
{
    ...
    esp_sleep_enable_ext1_wakeup(
        (1ULL << WAKE_GPIO_PREV) | (1ULL << WAKE_GPIO_NEXT),
...
power_mode_t power_mgmt_get_recommended_mode(void)
{
    ...
    if (current_config.enable_auto_deep_sleep && 
        inactive_time_ms >= current_config.deep_sleep_timeout_ms) {
        return POWER_MODE_DEEP_SLEEP;
    } else if (current_config.enable_auto_light_sleep && 
               inactive_time_ms >= current_config.light_sleep_timeout_ms) {
