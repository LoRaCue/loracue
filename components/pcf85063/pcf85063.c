...
{
    if (!rtc_handle) return ESP_ERR_INVALID_STATE;
    uint8_t buf[len + 1];
    buf[0] = reg;
    memcpy(&buf[1], data, len);
    return i2c_master_transmit(rtc_handle, buf, len + 1, 100);
}

static esp_err_t pcf85063_read_regs(uint8_t reg, uint8_t *data, size_t len)
{
    if (!rtc_handle) return ESP_ERR_INVALID_STATE;
    return i2c_master_transmit_receive(rtc_handle, &reg, 1, data, len, 100);
}
...
esp_err_t pcf85063_init(void)
{
    ...
    // Default to 400kHz
    return bsp_i2c_add_device(PCF85063_ADDR, 400000, &rtc_handle);
}
