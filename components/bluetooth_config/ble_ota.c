...
{
    if (!ota_gatts_if || !ota_control_handle || ota_conn_id == 0) return;
...
{
    if (!ota_gatts_if || !ota_progress_handle || ota_conn_id == 0) return;
...
void ble_ota_handle_control_write(const uint8_t *data, uint16_t len)
{
    ...
    switch (cmd) {
        case OTA_CMD_START:
            ...
            // Start timeout timer
            if (!ota_timeout_timer) {
                ota_timeout_timer = xTimerCreate("ota_timeout", pdMS_TO_TICKS(OTA_TIMEOUT_MS), 
