...
void ui_menu_update(ui_menu_t *menu, const char **item_names)
{
    ...
    if (menu->nav_up) {
        lv_obj_del(menu->nav_up);
        menu->nav_up = NULL;
    }
    if (menu->nav_down) {
        lv_obj_del(menu->nav_down);
        menu->nav_down = NULL;
    }
    if (menu->back_icon) {
        lv_obj_del(menu->back_icon);
        menu->back_icon = NULL;
    }
    ...
    for (int i = 0; i < UI_MENU_VISIBLE_ITEMS; i++) {
        ...
        if (item_index < menu->total_items) {
            ...
            bool has_nav_arrows =
                (menu->scroll_offset > 0) || (menu->scroll_offset + UI_MENU_VISIBLE_ITEMS < menu->total_items);
...
{
    if (!radio || index < 0 || index >= radio->item_count)
        return false;
